...
public class MoveLetter implements IMoveObject
{
  ...
  public final void move (final ICanvas3D sc3d,
      final ITransformGroup stg, final int... params)
  {
    final int trois = 3;
    final int cinq = 5;
    ...
    final IBranchGroup sbgOtherLetter = 
      S3DHelper.getScenePicker ().getNameAt (sc3d,
        "LetterBranchGroup", ax, ay);
    final IPoint3D sp3dPointer = sc3d.getPickedCoordinate (ax, ay);
    final boolean overBoard = BoundsChecker.isOverBoard (sp3dPointer);
    final boolean overChevalet = 
      TurnSpecificAlign.isOverChevalet (sp3dPointer, turn);
        ...
        && !stg.isTheSame (sbgOtherLetter.getTransformGroup ()))
    {
      ...
      final boolean otherLetterChevalet = 
        TurnSpecificAlign.isOverChevalet (
          stOtherLetter.getX (), stOtherLetter.getY (), stOtherLetter.getZ (),
          turn);
  ...
  private void alignOverBoard (final ICanvas3D sc3d, 
      final ITransformGroup stg, 
      final IPoint3D sp3dPointer, 
      final int turn)
  {
    final ITransform stCamera = 
      S3DHelper.newTransform ((Object [])null);
    ...
    final IPoint3D sp3dCamera = 
      S3DHelper.newPoint3d (new Double (stCamera.getX ()), 
                            new Double (stCamera.getY ()), 
                            new Double (stCamera.getZ ()));

    final IPoint3D sp3dLetter = 
      S3DHelper.newPoint3d (new Double (st.getX ()),
                            new Double (st.getY ()),
                            new Double (st.getZ ()));
