...
public final class SceneStepper
{
  ...
  private static final double []    STEPPING          = new double [] {
      ...
      final double [][] matrix)
  {
    ...
    return SceneStepper.stepToViewPoint (tg, actual, matrix, 0, quatre,
         0, quatre);
  }
  
  private static boolean stepToViewPoint (final ITransformGroup tg,
      final double [][] actual, final double [][] target, final int firstI,
      final int lastI, final int firstJ, final int lastJ)
  {
    ...
    final ITransform st = 
      S3DHelper.newTransform (new Object [] {actual,});
    st.normalize ();
    try
    {
      tg.setTransform (st);
