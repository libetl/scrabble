...
public abstract class AbstractDictionaryFactory implements IDictionaryFactory
{
  ...
  public final void load ()
  {
    ...
    try
    {
      final Class<?> c = Class.forName (Loader.class
          .getPackage ().getName ()
          + ".types." + extension2 + "To" + type2 + "Loader");
  ...
  public final void setFile (final String filename)
  {
    ...
    try
    {
      if (!this.getExtension ().toUpperCase ().equals (
          filename.substring (indExtension).toUpperCase ()))
      {
        throw new IllegalArgumentException ();
      }
      this.fileName = filename;
      this.l = new Locale (filename.substring (indCodePays + 1, indCodeLangue),
          filename.substring (indCodeLangue + 1, indExtension - 1));
      this.name = filename.substring (indFinFolder + 1, indCodePays);
      this.dict = DictionaryHelper.instantiateDictionary (this.type, this.l);
