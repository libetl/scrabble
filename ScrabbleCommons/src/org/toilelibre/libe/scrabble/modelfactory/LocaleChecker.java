...
public final class LocaleChecker
{
  ...
  public static Locale check (final Locale l, final String xmlFileName)
  {
    ...
    if (dotIndex != -1)
    {
      final String localeName = xmlFileName
          .substring (slashIndex + 1, dotIndex);
      final int underscoreIndex = localeName.indexOf ('_');
      if (underscoreIndex != -1)
      {
        final String language = localeName.substring (0, underscoreIndex);
        final String country = localeName.substring (underscoreIndex + 1);
