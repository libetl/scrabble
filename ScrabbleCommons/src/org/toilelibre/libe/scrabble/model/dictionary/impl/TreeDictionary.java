...
public class TreeDictionary implements Dictionary
{
  ...
  private boolean contains (final Node n, final String word)
  {
    return (word == null)
        || (word.length () == 0)
        || ((n.getChildNum (word.charAt (0)) != -1) && this.contains (n
            .getChild (word.charAt (0)), word.substring (1)));
  }
      ...
      final int numLettersMissing, final String debutMot)
  {
    ...
    for (final Node cN : n.getChildNodes ())
    {
      ...
      {
        final String word2 = word.substring (0, word.indexOf (cN.getValue ()))
            + word.substring (word.indexOf (cN.getValue ()) + 1);
        res.addAll (this.findMatches (word2, cN, numLettersMissing, debutMot
            + cN.getValue ()));
      } else if (numLettersMissing > 0)
      {
        res.addAll (this.findMatches (word, cN, numLettersMissing - 1, debutMot
            + cN.getValue ()));
      }
    }
    return res;
