...
public class Interaction
{
  ...
  public final void addEvent (final int instanceId, final String event,
      final String method)
  {
    Map<String, String> listEvents = this.events.get (new Integer (instanceId));
    if (listEvents == null)
    {
  ...
  {
    final String key = component + UIConstants.FILE_SEPARATOR + listenerType;
  ...
  public final void updateBindings ()
  {
    for (int i = 0; i < this.instances.size (); i += 1)
    {
      ...
      final Object component = FieldAccessor.get (this.bean,
          field);
    ...
    for (final String key : this.timers.keySet ())
    {
      ...
      timer = TimerBinder.bind (this.action, this.bean, this.timers.get (key)
          .get (0), this.timers.get (key).get (1));
