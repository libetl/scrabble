...
public final class ListenerBinder
{
  ...
  public static EventListener bind (final Object component,
      final String type, final Action action, final IUIBean bean,
      final Map<String, String> methods)
  {
    try
    {
      final Class<?> listenerClass = Class.forName ("java.awt.event." + type);
      final Class<?> listenerImplClass = Class.forName (ListenerBinder.class
          .getPackage ().getName ()
          + ".impl.UI" + type);
      final Constructor<EventListener> listenerConstructor = 
        (Constructor<EventListener>) listenerImplClass.getConstructor (
            new Class [] {Action.class, Map.class, IUIBean.class, });
      final Method mGet = component.getClass ().getMethod ("get" + type + "s",
          new Class [0]);
      final Method mAdd = component.getClass ().getMethod ("add" + type,
          new Class [] {listenerClass });
      final Method mRemove = component.getClass ().getMethod ("remove" + type,
          new Class [] {listenerClass });
