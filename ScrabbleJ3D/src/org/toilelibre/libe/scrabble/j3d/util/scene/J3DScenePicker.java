...
public final class J3DScenePicker implements IScenePicker
{
  ...
  public IBranchGroup getNameAt (final ICanvas3D sc3d,
      final String name, final int x, final int y)
  {
    ...
    final PickCylinderRay pcr = new PickCylinderRay ((Point3d) 
        ((J3DPoint3D)p).getObj (),
        new Vector3d (0.0, 0.0, 0.0), 0.05);

    final Locale bg = ((TransformGroup) 
        ((J3DTransformGroup)sc3d.getCameraTransformGroup ())
        .getImpl ()).getLocale ();
  ...
  public ITransformGroup getTransformGroupAt (
      final ICanvas3D sc3d, final String name, final int x, final int y)
  {
    final IBranchGroup sbg = 
      this.getNameAt (sc3d, name, x, y);
    if (sbg != null)
    {
  ...
  private static IBranchGroup lookForElement (
      final SceneGraphPath [] sgp, final String name)
  {
    ...
    if (sgp != null)
    {
      ...
      while (sbg == null && i < sgp.length)
      {
        ...
        while (sbg == null &&  j < element.nodeCount ())
        {
          ...
          {
            sbg = J3DScenePicker.findBranchGroup (jud, name, element, j);
          }
          j++;
        }
        i++;
  ...
  private static IBranchGroup findBranchGroup 
      (final J3DUserData jud, final String name, 
          final SceneGraphPath element, final int j)
  {
