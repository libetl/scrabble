...
public final class ChangeTurnAction extends Action {
    ...
    public ActionRedirect changeTurn (final IUIBean bean,
        ...
        session.put(
                ...
                new Integer((((Integer) session.get(ChangeTurnBean.TURN))
                        .intValue() + 1)
                        % ((Integer) session.get(ChangeTurnBean.NB_PLAYERS))
                                .intValue()));
    ...
    public ActionRedirect deployLetters (final IUIBean bean,
        ...
        final Session session = ScrabbleSessionsHandler.get();
        final int turn = ((Integer) session.get(ChangeTurnBean.TURN))
                .intValue();
        ILetterBranchGroup[][] lbgs = ctb.getLetterBranchGroups();
        if (lbgs == null) {
            lbgs = new ILetterBranchGroup[((Integer) session
                    .get(ChangeTurnBean.NB_PLAYERS)).intValue()][ChangeTurnBean.TRAY_LENGTH];
        }
        ...
        for (int i = 0 ; i < ChangeTurnBean.TRAY_LENGTH ; i++) {
            ...
            final Character c = (Character) ScrabbleBeansHelper.getComponent(
                    "pickLetter").execute(new Integer(turn), new Integer(i));
            if (c != null) {
                final int value = ((Integer) ScrabbleBeansHelper.getComponent(
                        "getLetterValue").execute(c)).intValue();
                lbg.affectLetter(c.charValue());
                lbg.setIdTray(i);
                lbg.setPoints(value);
                lbgs[turn][i] = lbg;
                lbg.setTransform(S3DHelper
                        .newTransform(new Object[] { TurnSpecificAlign.PIOCHE_VIEWPOINTS[turn], }));
            }
        ...
        for (int i = 0 ; i < ChangeTurnBean.TRAY_LENGTH ; i++) {
            if (lbgs[turn][i] != null) {
                gb.getSc3d().getScrabbleBranchGroup()
                        .addBranchGraph(lbgs[turn][i]);
            }
        }
    ...
    public ActionRedirect movement (final IUIBean bean,
        ...
        if (!SceneStepper.stepToViewPoint(this.stgToUse, this.goTo)) {
            ...
            try {
                this.methodToCallAtEnd.invoke(this, new Object[] { bean,
        ...
        for (int i = 0 ; i < ChangeTurnBean.TRAY_LENGTH ; i++) {
            if ((i == 0)
                ...
                if (TurnSpecificAlign.testLimitStepping(
                    ...
                    SceneStepper.stepWithFunction(lbgs[i].getTransformGroup(),
                            newPosition,
...

    {
        ...
        this.stgToUse = gb.getSc3d().getCameraTransformGroup();
        this.goTo = TurnSpecificAlign.READY_TO_PLAY_VIEWPOINTS[((Integer) session
                .get(ChangeTurnBean.TURN)).intValue()];
        try {
